'use client';

import React, { useEffect, useState } from 'react';

import { EducationItem } from '@/components/common';
import { Button } from '@/components/ui';

import { useLanguage } from '@/utils/LanguageContext';
import { getData } from '@/utils/getData';

type EducationItemType = {
  id: number;
  subTitle: string;
  description: string;
};

type EducationData = {
  title: string;
  link: string;
  education: EducationItemType[];
};

export const Education = () => {
  const { lang } = useLanguage();

  const [educationData, setEducationData] = useState<EducationData | null>(null);
  const [common, setCommon] = useState<any>(null);

  useEffect(() => {
    const loadData = async () => {
      const education = await getData('education', lang);
      const commonData = await getData('common', lang);

      setEducationData(education);
      setCommon(commonData);
    };

    loadData();
  }, [lang]);

  if (!educationData || !common) return null;

  return (
    <section className="w-full pb-[60px] pt-10 md:pb-[120px] md:pt-[60px] xl:pb-[140px] xl:pt-20">
      <div className="container">
        <div className="flex flex-col xl:flex-row">
          <div className="mb-10 flex flex-col md:mb-12 xl:mb-0 xl:mr-[153px] xl:max-w-[265px]">
            <h2 className="section-title mb-6 font-tenor text-accent md:text-start xl:mb-10">
              {educationData.title}
            </h2>

            <Button
              tag="a"
              href={educationData.link}
              accent={false}
              className="flex max-w-full justify-center text-sm font-normal md:max-w-[172px] md:items-center md:justify-center xl:max-w-[189px] xl:text-base"
            >
              {common.buttonsText.v2}
            </Button>
          </div>

          <ul>
            {educationData.education.map((item, index) => (
              <EducationItem
                key={item.id}
                item={item}
                index={index}
                isLast={index === educationData.education.length - 1}
              />
            ))}
          </ul>
        </div>
      </div>
    </section>
  );
};